...
const WeatherCard = ({ dateRange, city }) => {
  ...
  useEffect(() => {
    fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}/${start}/${end}?key=7PCYRZ5NLP3UT8RX39YGEFEQJ`,
    )
      .then((res) => {
        return res.json();
      })
  ...
  return <WeatherParse days={weatherData} />;
};
